...
//            'addons'  => sprintf($link, $this->pluginMeta->getPaidAddonsUrl(),  __('Paid add-ons', 'wp-testing')),
            'settings' => sprintf($link, get_site_url().'/wp-admin/edit.php?post_type=wpt_test&page=wpt_feedback_paid_addons', 'تنظیمات', 'wp-testing'),
        );
    }
    ...
    public function addPages()
    {
        ...
        $hook = $this->wp->addSubmenuPage(
            ...
            'activate_plugins',
            'wpt_Payments',
            array($this, 'renderPaymentsPages')
        );
        $hook = $this->wp->addSubmenuPage(
            'edit.php?post_type=wpt_test',
            'ماژول ها',
            ...
            'activate_plugins',
            'wpt_modular',
            array($this, 'render_modular_Page')
        );
...
//        $this->wp->addAction('load-' . $hook, array($this, 'loadGetSupport'));
    ...
    {
        $this->output('Passing/Browser/settings');
    }
    public function render_modular_Page()
    {
        $this->output('Passing/Browser/modular');
    }
    public function renderPaymentsPages()
    {
        $this->output('Passing/Browser/Payments');
    }
